...
static void check_mapping_monotonic(LogParserInterface* lpi) {
  for (size_t i = 1; i < lpi->local_to_global_id.size(); i++) {
    REQUIRE(lpi->local_to_global_id[i] > lpi->local_to_global_id[i - 1]);
  }
}
...
TEST_CASE("jumpToLocalLine - forward small hop") {
  ...
  SECTION("jump just past block end") {
    lpi->jumpToLocalLine(20);
    line_info_t info = lpi->getLine(20);
    REQUIRE(info.line != nullptr);
    REQUIRE(info.line->line_num == 20);
    // Block should now contain line 20
    REQUIRE(lpi->block.first_line_local_id <= 20);
    REQUIRE(lpi->block.first_line_local_id + lpi->block.lines.size() > 20);
  }
  SECTION("jump to last line of initial block is a no-op") {
    line_t first_before = lpi->block.first_line_local_id;
    size_t size_before = lpi->block.lines.size();
    lpi->jumpToLocalLine(14);
    REQUIRE(lpi->block.first_line_local_id == first_before);
    REQUIRE(lpi->block.lines.size() == size_before);
  }

  check_mapping_monotonic(lpi);
  teardown();
}
...
TEST_CASE("jumpToLocalLine - forward into uncharted territory") {
  ...
  lpi->jumpToLocalLine(50);

  line_info_t info = lpi->getLine(50);
  REQUIRE(info.line != nullptr);
  REQUIRE(info.line->line_num == 50);
  REQUIRE(lpi->block.first_line_local_id <= 50);
  REQUIRE(lpi->block.first_line_local_id + lpi->block.lines.size() > 50);
  check_mapping_monotonic(lpi);
  teardown();
}
...
TEST_CASE("jumpToLocalLine - jump to LINE_T_MAX reaches EOF") {
  ...
  line_t last_local = lpi->block.first_line_local_id + lpi->block.lines.size() - 1;
  line_info_t info = lpi->getLine(last_local);
  REQUIRE(info.line != nullptr);
...
TEST_CASE("jumpToLocalLine - backward to previously visited line") {
  ...
  lpi->jumpToLocalLine(40);
  REQUIRE(lpi->getLine(40).line != nullptr);

  SECTION("jump back within one block_size distance") {
    lpi->jumpToLocalLine(30);
    line_info_t info = lpi->getLine(30);
    REQUIRE(info.line != nullptr);
    REQUIRE(info.line->line_num == 30);
    REQUIRE(lpi->block.first_line_local_id <= 30);
  }
  SECTION("jump back far (more than block_size away)") {
    lpi->jumpToLocalLine(5);
    line_info_t info = lpi->getLine(5);
    REQUIRE(info.line != nullptr);
    REQUIRE(info.line->line_num == 5);
    REQUIRE(lpi->block.first_line_local_id <= 5);
    REQUIRE(lpi->block.first_line_local_id + lpi->block.lines.size() > 5);
  }
...
TEST_CASE("jumpToLocalLine - jump to 0 from end of file") {
  ...
  line_info_t info = lpi->getLine(0);
  REQUIRE(info.line != nullptr);
  REQUIRE(info.line->line_num == 0);
  REQUIRE(info.flags & INFO_IS_FIRST_LINE);
  REQUIRE(lpi->block.first_line_local_id == 0);
...
TEST_CASE("jumpToLocalLine - round-trip end-beginning-end-beginning") {
  ...
  lpi->jumpToLocalLine(LINE_T_MAX);
  REQUIRE(lpi->block.contains_last_line);
  line_t last_local_1 = lpi->block.first_line_local_id + lpi->block.lines.size() - 1;
  REQUIRE(last_local_1 == TOTAL_LINES - 1);
  size_t mapping_size_after_first_end = lpi->local_to_global_id.size();
  ...
  REQUIRE(lpi->local_to_global_id.size() == mapping_size_after_first_end);
  check_mapping_monotonic(lpi);

  // Second trip to end — should produce same block state as first
  lpi->jumpToLocalLine(LINE_T_MAX);
  REQUIRE(lpi->block.contains_last_line);
  line_t last_local_2 = lpi->block.first_line_local_id + lpi->block.lines.size() - 1;
...
TEST_CASE("jumpToLocalLine - round-trip line content is stable") {
  ...
  lpi->jumpToLocalLine(30);
  std::string line30_first(lpi->getLine(30).line->raw_line);
  ...
  // Back to 30
  lpi->jumpToLocalLine(30);
  std::string line30_second(lpi->getLine(30).line->raw_line);
  ...
  // Back to 30 again
  lpi->jumpToLocalLine(30);
  std::string line30_third(lpi->getLine(30).line->raw_line);
...
TEST_CASE("jumpToLocalLine - filtered: forward small hop") {
  setup();
  auto* lpi = make_info_filtered_lpi(5);
  // Initial block holds filtered lines 0-4

  lpi->jumpToLocalLine(8);
  line_info_t info = lpi->getLine(8);
  REQUIRE(info.line != nullptr);
...
TEST_CASE("jumpToLocalLine - filtered: round-trip") {
  ...
  size_t mapping_size = lpi->local_to_global_id.size();

  // To beginning
  lpi->jumpToLocalLine(0);
  REQUIRE(lpi->getLine(0).line != nullptr);
  ...
  REQUIRE(lpi->local_to_global_id.size() == mapping_size);
  check_mapping_monotonic(lpi);

  // To beginning again
  lpi->jumpToLocalLine(0);
  REQUIRE(lpi->getLine(0).flags & INFO_IS_FIRST_LINE);
...
TEST_CASE("jumpToLocalLine - filtered: all lines accessible after full traversal") {
  ...
  for (int i = 0; i < FILTERED_LINES; i++) {
    line_info_t info = lpi->getLine(i);
    REQUIRE(info.line != nullptr);
